...
function getColor( d )
{
  return d > 8000 ? '#800026' : d > 5000 ? '#BD0026' : d > 1000 ? '#E31A1C' : d > 400 ? '#FC4E2A' : d > 300 ? '#FD8D3C' : d > 200 ? '#FEB24C' : d > 100 ? '#FED976' : '#FFEDA0';
}
...
function setMap()
{
  ...
  {
    this._div.innerHTML = '<h4>Traffic Congestion</h4>' + ( props ? '<b>' + props + '</b><br />' + props + ' people ' : 'Hover over a state' );
  };
  info.addTo( map );
...
function createLegend()
{
  ...
  legend.onAdd = function( map )
  {
    ...
    var div = L.DomUtil.create( 'div', 'info legend' ), incidents = [ 0, 100, 200, 300, 400, 1000, 5000, 8000 ], labels = [];

    // loop through our density intervals and generate a label with a
    // colored square for each interval
    for ( var i = 0; i < incidents.length; i++ )
    {
      div.innerHTML += '<i style="background:' + getColor( incidents[i] + 1 ) + '"></i> ' + incidents[i] + ( incidents[i + 1] ? '&ndash;' + incidents[i + 1] + '<br>' : '+' );
    }
    return div;
  };
...
function infoPanel()
{
  ...
  info.update = function( props )
  {
    this._div.innerHTML = '<h4>SAN FRANSICO Crime</h4>' + ( props ? '<b>' + props.name + '</b><br />' + props.density + ' people / mi<sup>2</sup>' : 'Hover over a state' );
  };
...
function drawPlot()
{
  ...
  ocpu.seturl( "http://localhost:9075/ocpu/library/stats/R" );
